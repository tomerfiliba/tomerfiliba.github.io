...
mission-critical code can't allow for the occasional `NameError` or `None has not member __len__` 
...
<a href="http://dlang.org"><img src="http://tomerfiliba.com/static/res/2014-03-27-dlogo.png" title="Dlang" class="blog-post-image"></a>

System programming is a vast ocean of specifics, technicalities and constraints, imposed by your specific use case.
...
dreamed to be. It offers better syntax, much faster compilation time, (optional) garbage collection, highly
...
(like closures) with low-level ones (naked function in inline assembly) to produces efficient code, has 
...
function execution. Whoo, that was long.
...
I used it in many of my commercial products, where I needed to generate code.
...
In other words, you won't have to write a toy parser in Construct and then rewrite (by hand) it in `C++`.
...
The real power of Construct comes from adapters, which let you operate at the representational ("DOM") level
of data, rather on its binary form. That required lambdas, closures and other higher-level concepts that C lacks.
...
if I write `myCtx.foo` and `myCtx` doesn't define `foo`, the code is rewritten as `myCtx._curr.foo`.

As we travel along constructs, we link the current context with its ancestor (`_`). This means that for each 
combination of constructs, and each level of nesting, we get a uniquely-typed context. At runtime, this context is 
...
is mixin'ed with `ctx`, yielding `ctx._.length`. If we refered to the wrong path or misspelled anything,
