---
layout: blogpost
title: Javaism And Large Scale Projects
draft: true
...
a bit on coding style. Due to it's length, I've split it over several installments; the first 
...
* Speaking of the Thread constructor, it takes so many optional arguments (``group``?!), but you 
  have to imperatively call ``setDaemon()`` afterwards (instead of passing ``daemon = True`` in 
  construction time). Why? Because Java doesn't have *keyword arguments*, so adding an additional 
  argument you multiply the number of overloaded constructors (exponential), while get/set 
...
* Also, you first *instantiate* the thread object, then ``start()`` it... where's the sense in that?
  ...
  as an internal state of the object -- it's a common Java practice. 
