...
8.  **D has poor adoption** and lacking the codebase of C/C++ and even Go. But as I explained, in system programming
    you can almost never rely on external code as-is (it might allocate/block/be inefficient) so you end 
    up living in a rather closed world. On the other hand, D has C ABI so with a bit of porting effort 
    (converting H files), you get virtually everything. 
    
    D serves us well so far, and it eliminates the all too common write-the-core-in-C-but-everything-else-in-Python 
    paradigm.
    
    People also said (perhaps even *boasted*) that are **no success stories** or large projects using D. 
    First of all, that's not true. Google around. But being a startup, my colleagues and I are not afraid of 
    hard work and living on the edge. And hopefully you'll hear about a D success story in a year or two :)
